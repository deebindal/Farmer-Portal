...
module.exports.addCrop_sms = async function(req, res) {
  ...
  try {
    ...
    const user = await userModel.find({ phone: number,role:"farmer"})
    console.log(user)
    ...
    const obj = {
      ...
      price: data[2],
      quantity: data[3],
      description: data.slice(4).join(" "),
      phone: number,
      seller:user[0]._id
    };
    const crop = await cropModel.create(obj);
    const smsToSend = `Your data has been saved successfully in our database`;
    console.log("data saved")
    sendSMS(number, smsToSend);
